...
TF1* fit_xy_res(double &x, double &xe, double &max, bool do_fit, int idel, TCanvas *tc, TH1F* h, const char *meth, const char *var_tag, const char *det_tag, const char *var, const char *det_desc) {
  ...
  if (do_fit) {
    ...
    tt->DrawLatex(0.65,0.5, Form("#delta = %d%%",idel-6 ));
...
void plot_pres(int CASE=0) {
  ...
  if(CASE == 0 ) {
    ...
    fName = Form("%sms10_ds11_seg7.0_xy0.5mm_dth10_dph50_p1.30.root",path);
    idx_dia = 6; idx_had = 7;    
    s_tag = "3ai";
  } else if (CASE == 9) { // 3aII
    fName = Form("%sms10_ds11_seg3.5_xy0.5mm_dth10_dph50_p1.30.root",path);
    idx_dia = 6; idx_had = 7;
    s_tag = "3aii";
  } else if (CASE == 10) { // 3aIII
    fName = Form("%sms10_ds11_seg1.0_xy0.5mm_dth10_dph50_p1.30.root",path);
    idx_dia = 6; idx_had = 7;    
    s_tag = "3aiii";
  } else if (CASE == 11) { // 3b
  ...
  const int ndel =  13;
  const int nmeth = 2; // 0->TDR, 1->Minuit
  const int nmeth_max = 2;
  
  // X resolution distribution maximia
  Double_t xmax_dia[nmeth][ndel] = {{0.0}};
  Double_t ymax_dia[nmeth][ndel] = {{0.0}};
  Double_t xmax_had[nmeth][ndel] = {{0.0}};
  Double_t ymax_had[nmeth][ndel] = {{0.0}};
  ...
  TH1F *hpres[nmeth][ndel];
  TH1F *hxres_dia[nmeth][ndel];
  ...
  for (int idel=0; idel<ndel; ++idel) {
    ...
    for (int imeth=0; imeth<nmeth_max; ++imeth) {
      hpres[imeth][idel] = (TH1F*) def_hist(Form("h_pres_dp%d_%s",idel,cmeth[imeth]), 1000, 1.16, 1.4, idel!=9?idel+1:42);      
  ...
  for (int ient=0; ient<nent; ++ient) {
    ...
    t->GetEntry(ient);
    int idel = floor(dp[0]+0.5) + 6;
    if (ient%10000==0) {
      //if (idel==5)
      cout << "ient= " << ient << "/"<< nent << " p[0]= " << p[0] << " pr[0]= " << pr[0] << " dp[0] = " << dp[0] << " idel= " << idel << endl;
    }

    if (idel<0 || idel>13) cout << "WTF! idel = " << idel << endl;
    //cout << "p[0]= " << p[0] 
    if (sr[0] != 0 ) continue;
    ...
    //hpres[1][idel]->Fill(pr[3]);
    //hxres_dia[1][idel]->Fill(xr[4]-x[idx_dia]);
    //hyres_dia[1][idel]->Fill(yr[4]-y[idx_dia]);
    //hxres_had[1][idel]->Fill(xr[5]-x[idx_had]);
    //hyres_had[1][idel]->Fill(yr[5]-y[idx_had]);
    //
    //hpres[0][idel]->Fill(pr[0]);
    //hxres_dia[0][idel]->Fill(xr[1]-x[idx_dia]);
    //hyres_dia[0][idel]->Fill(yr[1]-y[idx_dia]);
    //hxres_had[0][idel]->Fill(xr[2]-x[idx_had]);
    //hyres_had[0][idel]->Fill(yr[2]-y[idx_had]);
  ...
  //for (int idel=0; idel<ndel; ++idel) {
  //  for (int imeth=0; imeth<nmeth_max; ++imeth) {
  //    cout << (hxres_dia[imeth][idel]->GetMean() - 3*hxres_dia[imeth][idel]->GetRMS()) << "  "
  //	   << (hyres_dia[imeth][idel]->GetMean() - 3*hyres_dia[imeth][idel]->GetRMS()) << "  "
  //	   << (hxres_had[imeth][idel]->GetMean() - 3*hxres_had[imeth][idel]->GetRMS()) << "  "
  //	   << (hyres_had[imeth][idel]->GetMean() - 3*hyres_had[imeth][idel]->GetRMS()) << endl;
  //    
  //    outf << (hxres_dia[imeth][idel]->GetMean() - 3*hxres_dia[imeth][idel]->GetRMS()) << "  "
  //	   << (hyres_dia[imeth][idel]->GetMean() - 3*hyres_dia[imeth][idel]->GetRMS()) << "  "
  //	   << (hxres_had[imeth][idel]->GetMean() - 3*hxres_had[imeth][idel]->GetRMS()) << "  "
  //	   << (hyres_had[imeth][idel]->GetMean() - 3*hyres_had[imeth][idel]->GetRMS()) << endl;
  //  }
  ...
  TF1 *fxres_dia[nmeth][ndel];
  TF1 *fyres_dia[nmeth][ndel];
  TF1 *fxres_had[nmeth][ndel];
  TF1 *fyres_had[nmeth][ndel];  
  double d_xres_dia[nmeth][13] = {{0.0}}, d_xres_dia_e[nmeth][13] = {{0.0}};
  double d_yres_dia[nmeth][13] = {{0.0}}, d_yres_dia_e[nmeth][13] = {{0.0}};
  double d_xres_had[nmeth][13] = {{0.0}}, d_xres_had_e[nmeth][13] = {{0.0}};
  double d_yres_had[nmeth][13] = {{0.0}}, d_yres_had_e[nmeth][13] = {{0.0}};
  ...
  if (do_fit) {
    ...
    for (int imeth=0; imeth<nmeth_max; ++imeth) {
      ...
      tge_pres[imeth]= new TGraphErrors(13, d_del, d_pres[imeth], d_del_e, d_pres_e[imeth]);
      style_tgraph(tge_pres[imeth], Form("tge_pres_%s",cmeth[imeth]), Form("Momentum Resolution (%s); mom offset(%%); mom res (%%)",cmeth[imeth]), meth_color[imeth], 20, 1.); 

      tge_xres_dia[imeth]= new TGraphErrors(13, d_del, d_xres_dia[imeth], d_del_e, d_xres_dia_e[imeth]);
      style_tgraph(tge_xres_dia[imeth], Form("tge_xres_dia_%s",cmeth[imeth]), "X Pos Resolution (mm); mom offset(%%); Position resolution (mm)", meth_color[imeth], det_marker[0], det_marker_size[0]);
      
      tge_yres_dia[imeth] = new TGraphErrors(13, d_del, d_yres_dia[imeth], d_del_e, d_yres_dia_e[imeth]);
      style_tgraph(tge_yres_dia[imeth], Form("tge_yres_dia_%s",cmeth[imeth]), "Y Pos Resolution (mm); mom offset(%%); Position resolution (mm)", meth_color[imeth], det_marker[0], det_marker_size[0]);
      
      tge_xres_had[imeth] = new TGraphErrors(13, d_del, d_xres_had[imeth], d_del_e, d_xres_had_e[imeth]);
      style_tgraph(tge_xres_had[imeth], Form("tge_xres_had_%s",cmeth[imeth]), "X Pos Resolution (mm); mom offset(%%); Position resolution (mm)", meth_color[imeth], det_marker[1], det_marker_size[1]);

      tge_yres_had[imeth] = new TGraphErrors(13, d_del, d_yres_had[imeth], d_del_e, d_yres_had_e[imeth]);
      style_tgraph(tge_yres_had[imeth], Form("tge_yres_had_%s",cmeth[imeth]), "Y Pos Resolution (mm); mom offset(%%); Position resolution (mm)", meth_color[imeth], det_marker[1], det_marker_size[1]);      
