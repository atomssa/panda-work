...
void effcth(){
  ...
  const int nplab = 3;
  const double plab[nplab] = {5.5, 8., 12.};
  ...
  TLegend *legend3 = new TLegend(0.1, 0.7, 0.9, 0.9);
  legend3->SetFillStyle(0);
  legend3->SetBorderSize(0);
  legend3->SetTextSize(0.05);

  const char* toru[4] = {"t","t","u","u"};

  TMultiGraph *tmg = new TMultiGraph("tmg",";cos(#theta^{e^{+}}_{J/#psi});Signal Reco #varepsilon_{tot}");
  ...
  for (int iplab = nplab-1; iplab >=0; --iplab) {
    ...
    total[iplab] = (TH1F*) f[iplab]->Get(Form("epeff/hepcosth_jpsi_mc_itu%d",itu));
    passed[iplab] = (TH1F*) f[iplab]->Get(Form("epeff/hepcosth_jpsi_rec_itu%d",itu));
  ...
  for (int iplab=0; iplab < nplab; ++iplab) {
    if (iplab==0)
      legend3->AddEntry(tge[iplab], Form("p^{LAB}_{#bar{p}} = %3.1f GeV/c, %5.3g < %s[GeV] < %5.3g",plab[iplab], tvalidmin[iplab], toru[itu], tvalidmax[iplab]), "pl");
    else
      legend3->AddEntry(tge[iplab], Form("p^{LAB}_{#bar{p}} = %3.1f GeV/c, %3.1f < %s[GeV] < %5.3g",plab[iplab], tvalidmin[iplab], toru[itu], tvalidmax[iplab]), "pl");
  }

  //legend3->AddEntry(tge[0], Form("p^{LAB}_{#bar{p}} = %3.1f GeV/c, %5.3f < t < %5.3f",plab[0], tvalidmin[0], tvalidmax[1]), "pl");
  ...
  tceff->Print(Form("efficiency_vs_costh_%s.pdf",toru[itu]));
