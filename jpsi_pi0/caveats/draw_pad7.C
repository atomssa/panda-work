void draw_pad7(TCanvas *tc, int ic, const char* fname_mu, const char* fname_e, double pt, double thmin, double thmax) {
  int ipad  = ic+1;
  ...
  //tt->SetTextColor(4);
  //tt->DrawLatex(0.57,0.52,Form("Unwtd. corr"));

  tt->SetTextColor(2);
  tt->DrawLatex(0.60,0.42,Form("KF+corr."));

  tt->SetTextColor(1);
  tt->DrawLatex(0.63,0.32,Form("KF"));

  tt->SetTextColor(1);
  tt->DrawLatex(0.57,0.82,Form("p_{T}= %4.2g GeV/c",pt));
  tt->DrawLatex(0.57,0.72,Form("%3.0f#circ < #theta < %3.0f#circ",thmin,thmax));
