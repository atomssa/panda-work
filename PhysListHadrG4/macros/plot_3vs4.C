void plot_3vs4() {
  ...
  TCanvas *tc_3vs4_pip = new TCanvas("tc_3vs4_pip","tc_3vs4_pip");
  tc_3vs4_pip->Divide(3,2);

  TCanvas *tc_3vs4_pim = new TCanvas("tc_3vs4_pim","tc_3vs4_pim");
  ...
  for (int imom=0; imom<nmom; ++imom) {
    ...
    for (int ipl=0; ipl<npl; ++ipl) {
      string fname_pip = Form("output/hist_%s_MOM_%3.1f_pip.root",tags[pl[ipl]].c_str(),mom_array[imom]);
    ...
    for (int ipl=0; ipl<npl; ++ipl) {
      string fname_pim = Form("output/hist_%s_MOM_%3.1f_pim.root",tags[pl[ipl]].c_str(),mom_array[imom]);
  ...
  tc_3vs4_pip->Print("figs/g3vsg4_pip.eps");
  tc_3vs4_pim->Print("figs/g3vsg4_pim.eps");
