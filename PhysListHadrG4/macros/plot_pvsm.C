void plot_pvsm(int itag) {
  ...
  TFile* f_pim[nmom];
  TLegend* tl_pm[nmom];
  TCanvas *tc_pm = new TCanvas("tc_pm","tc_pm",1400,1000);
  tc_pm->Divide(3,2);
  ...
  for (int imom=0; imom<nmom; ++imom) {
    ...
    if (imom == 2 && (itag==0||itag==1||itag==2||itag==5||itag==6)) {
    ...
    /*
    if (imom == 2) {
      if (itag==0||itag==1||itag==2||itag==5||itag==6) {
  ...
  tc_pm->Print(Form("figs/pvsm_%s.eps",tags[itag].c_str()));
