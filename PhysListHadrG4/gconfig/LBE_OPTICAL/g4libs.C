...
/// Macro function for loading VGM libraries.
  ...
  if (isSet("USE_VGM")) { 
    cout << "$$$$LocalErmias$ Loading VGM libraries ... " << endl;
...
/// and external packages: CLHEP, graphics drivers, .. used by Geant4
  ...
  cout << "$$$$LocalErmias$ Loading Geant4 global libraries ... My Macro (Ermias) " << endl;
...
//  printf(" line = %s \n", line.Data());
  ...
  if ( isLibrary("libg4root") ) {   
    cout << "$$$$LocalErmias$ Loading g4root library ..." << endl;
  ...
  cout << "$$$$LocalErmias$ Loading libraries ... finished " << endl;
}
