...
void HistFillerTask::initialize_hists() {
  ...
  h_extra_clust_dphi_dthe = new TH2F("h_extra_clust_dphi_dthe","Distance of extra clusturs from main cluster",200,-8,8,200,-4,4);
  h_extra_bump_dphi_dthe = new TH2F("h_extra_bump_dphi_dthe","Distance of extra bumps from main cluster",200,-8,8,200,-4,4);
  h_extra_reco_hit_dphi_dthe = new TH2F("h_extra_reco_hit_dphi_dthe","Distance of extra bumps from main cluster",200,-8,8,200,-4,4);
...
void HistFillerTask::Exec(Option_t* opt)
{
  ...
  for (int iclust=0; iclust<nclust; ++iclust) {
    ...
    h_all_clust_energy->Fill(tmp_clust->energy()/trk_energy);
  ...
  h_clust_tot_energy->Fill(energy_tot_clust/trk_energy);    
  ...
  if (nclust > 0 ) { 
    ...
    const double clust_phi_deg = clust_phi*180./TMath::Pi();
    const double clust_the_deg = clust_the*180./TMath::Pi();
    if (inacc) {
      h_energy->Fill(clust_energy/trk_energy);
      h_dphi->Fill(trk_phi_deg-clust_phi_deg);
      h_dthe->Fill(trk_the_deg-clust_the_deg);
    }
    h_energy_b->Fill(clust_energy/trk_energy);

    for (int iclust=0; iclust<nclust; ++iclust) {
      if (iclust==i_main_clust) continue;
      PndEmcCluster *tmp_clust = (PndEmcCluster*)clust_array->At(iclust);
      const double _clust_energy = tmp_clust->energy();
      ...
      h_extra_clust_energy->Fill(_clust_energy/trk_energy);
    }
  ...
  for (int ibump=0; ibump<nbump; ++ibump) {
    ...
    h_all_bump_energy->Fill(tmp_bump->energy()/trk_energy);
  ...
  h_bump_tot_energy->Fill(energy_tot_bump/trk_energy);    
  if (debug>3) cout << endl;
  if (nbump > 0 ) { 
    ...
    for (int ibump=0; ibump<nbump; ++ibump) {
      ...
      PndEmcBump *tmp_bump = (PndEmcBump*)bump_array->At(ibump);
      const double _bump_energy = tmp_bump->energy();
      ...
      h_extra_bump_energy->Fill(_bump_energy/trk_energy);
    }
  ...
  for (int ireco_hit=0; ireco_hit<nreco_hit; ++ireco_hit) {
    ...
    h_all_reco_hit_energy->Fill(tmp_reco_hit->GetEnergyCorrected()/trk_energy);
  ...
  h_reco_hit_tot_energy->Fill(energy_tot_reco_hit/trk_energy);
  h_reco_hit_tot_energy_corr->Fill(energy_tot_reco_hit_corr/trk_energy);    
  if (debug>3) cout << endl;
  if (nreco_hit > 0 ) { 
    ...
    for (int ireco_hit=0; ireco_hit<nreco_hit; ++ireco_hit) {
      ...
      PndEmcRecoHit *tmp_reco_hit = (PndEmcRecoHit*)reco_hit_array->At(ireco_hit);
      const double _reco_hit_energy=tmp_reco_hit->GetEnergy();
      ...
      h_extra_reco_hit_energy->Fill(_reco_hit_energy/trk_energy);
    }
...
{	
  ...
  h_extra_clust_dphi_dthe->Write();
  h_extra_bump_dphi_dthe->Write();
  h_extra_reco_hit_dphi_dthe->Write();

  h_extra_clust_energy->Write();
  h_extra_bump_energy->Write();
  h_extra_reco_hit_energy->Write();

  h_all_clust_energy->Write();
  h_all_bump_energy->Write();
  h_all_reco_hit_energy->Write();

  h_clust_tot_energy->Write();
  h_bump_tot_energy->Write();
  h_reco_hit_tot_energy->Write();
  h_reco_hit_tot_energy_corr->Write();
