...
do
	...
	if [[ $diff_cnt -gt 0 ]]; then
	    echo "Patch file $SRC_FILE different from $PNDROOT_DIR version. Diff:"
	    nbkp=$(ls $PNDROOT_DIR/$SRC_FILE.org.* 2>/dev/null | wc -l| tr -d '[[:space:]]')
	    git --no-pager diff -b -w --ignore-blank-lines --ignore-space-at-eol $PNDROOT_DIR/$SRC_FILE $PATCH_DIR/$SRC_FILE
